...
const Register = () => {
    ...
    const{setUsername:setlogUsername,setId} =useContext(UserContext);
    
    async function register(ev) {
        const url = loginButton === 'Register'?'/register':'/login' 
        ev.preventDefault();
                                ...
                                <div class="space-y-5">
                                    ...
                                    <div>
                                        <div class="flex items-center justify-between">
                                           ...
                                           {loginButton === 'Login' && (<a href="#" title="" class="text-sm font-medium text-orange-500 transition-all duration-200 hover:text-orange-600 focus:text-orange-600 hover:underline"> Forgot password? </a>
                                        )} </div>
                                        <div class="mt-2.5 relative text-gray-400 focus-within:text-gray-600">
                                            <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                                                <svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    ...
                                    {loginButton === 'Login' && (
                                        <p class="text-base text-gray-600">Don't have an account? <a href="#" onClick={()=>setLoginButton('Register')} title="" class="font-medium text-orange-500 transition-all duration-200 hover:text-orange-600 hover:underline"> Register here</a></p>
                                    )}
                                    {loginButton === 'Register' && (
                                        <p class="text-base text-gray-600">Already have an account? <a href="#" onClick={()=>setLoginButton('Login')} title="" class="font-medium text-orange-500 transition-all duration-200 hover:text-orange-600 hover:underline"> login here</a></p>
                                    )}      
                                    </div>
                                </div>
                            </form>
